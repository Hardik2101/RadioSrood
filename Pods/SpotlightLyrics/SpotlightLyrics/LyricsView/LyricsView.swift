...
open class LyricsView: UITableView, UITableViewDataSource, UITableViewDelegate {
    ...
    public func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return lineSpacing + cellViewModel.calcHeight(containerWidth: self.bounds.width) + 10
    }
    ...
    internal func scroll(toTime time: TimeInterval, animated: Bool) {
        ...
        for (lyricsIndex, _) in lyricsViewModels.enumerated() {
            lyricsViewModels[lyricsIndex].highlighted = lyricsIndex <= index - 1
        }
    }
}
