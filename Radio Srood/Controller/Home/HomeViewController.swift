...
//            isPurchaseSuccess = false
    ...
    private func handleHomeHeaderArrayValue() {
        ...
        if playList.count <= 0 {
            homeHeaderArray.remove(at: 5)
        }
//        if nativeAd.count > 0 {
            homeHeaderArray.insert("Native Ad First", at: 3)
//            if nativeAd.count >= 2 {
...
//        loadNativeAd()
    ...
    private func setHeaderData(headerTitle: String) -> UIView {
        ...
        lblTitle.font = UIFont(name: "Kohinoor Telugu Light", size: 16)
...
extension HomeViewController: GADInterstitialDelegate {
    ...
    func interstitialDidDismissScreen (_ interstitial: GADInterstitial) {
        ...
        switch homeHeader {
        case .newReleases, .trending, .popularTracks, .playlists, .featuredArtist:
